...
import Config, { AppRuntimeConfig } from "lib/config";
...
const loadConfig = async (rawConfig: AppRuntimeConfig, database: Option<Database>): Promise<AppRuntimeConfig> => {
...
const setupAppDependencies = (): Promise<Result<AppDependencies, Error>> => buildResult(async () => {
    ...
    database.map(async (db) => {
        const result = await db.setAppConfig(config);
        result.mapErr((err) => {
            console.error(err);
        })
    });
