...
describe("services/database/postgres", () => {
    describe("getAppConfig", () => {
        it("maps a found row, defaulting null tts_region/tts_api_key/admins", async () => {
            ...
            const db = database.newDatabase(asSql(fake), DEFAULT_ADMINS);
    ...
    describe("setAppConfig", () => {
        it("gates the write on the caller being an admin via a single admins-array query", async () => {
            const fake = createFakeSql();
            fake.queueResponse([]);
            const db = database.newDatabase(asSql(fake), DEFAULT_ADMINS);

            const result = await db.setAppConfig(fakeAppConfig({ admins: ["1", "2"] }), "1");

            expect(result.isOk()).toBe(true);
            expect(fake.calls).toHaveLength(1);
            expect(fake.calls[0].text).toContain("ANY(app_runtime_info.admins)");
            expect(fake.calls[0].text).not.toContain("IS NULL");
        });
    });
            ...
            fake.queueResponse([
                {
                    ...
                    muted: true,
                    join_message: { prefix: "a", content: "b" },
                    leave_message: null,
                },
            ]);
            const db = database.newDatabase(asSql(fake), DEFAULT_ADMINS);
